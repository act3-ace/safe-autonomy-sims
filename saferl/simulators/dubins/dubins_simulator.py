...
import saferl.simulators.dubins.backend.platforms as b
...
class DubinsPlatformConfigValidator(BaseModel):
    ...
    def check_position_len(cls, v, field):
        if len(v) != 3:
            raise ValueError(f"{field.name} provided to DubinsPlatformConfigValidator is not length 3")
        return v


class DubinsSimulatorResetValidator(BaseSimulatorResetValidator):
    agent_initialization: typing.Optional[typing.Dict[str, DubinsPlatformConfigValidator]] = {
        "blue0": DubinsPlatformConfigValidator(position=[0, 1, 2], velocity=[0, 0, 0])
    }


# TODO: Create combined CWH/Dubins saferl simulator superclass

class Dubins2dSimulator(BaseSimulator):
    ...
    def __init__(self, **kwargs):
        ...
        self.sim_entities = {agent_id: b.Dubins2dPlatform(name=agent_id) for agent_id in self.config.agent_configs.keys()}
