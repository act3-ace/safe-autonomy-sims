...
from act3_rl_core.simulators.base_available_platforms import \
    BaseAvailablePlatformTypes
...
class CWHAvailablePlatformTypes(BaseAvailablePlatformTypes):
    ...
    def ParseFromNameModel(cls, config: dict) -> CWHAvailablePlatformTypes:
        ...
        if "name" not in config:
            raise RuntimeError(
                "Attempting to parse a PlatformType from name/model config, but both are not given!"
            )
        ...
        raise RuntimeError(
            f'name: {config["name"]} and model: {config["model"]} did not match a known platform type'
        )


PluginLibrary.AddClassToGroup(CWHAvailablePlatformTypes,
                              "CWHSimulator_Platforms",
