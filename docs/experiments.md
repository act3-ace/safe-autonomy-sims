...
1. To create the state transistion dynamics, extend `BaseDynamics` (or `BaseODEDynamics` if using A,B matrices) from `safe_autonomy_simulation.base_models`. The child class must generate A and B matrices in its **init**() method and pass them to its super's constructor. See `safe_autonomy_simulation.spacecraft.point_model.CWHDynamics` for an example.
2. To create the entity containing the new Dynamics, extend `safe_autonomy_simulation.base_models.BaseEntity` and pass an instance of the new Dynamics class to super() constructor inside the Entity's **init**() method. See `safe_autonomy_simulation.spacecraft.point_model.CWHSpacecraft` for an example.
