...
test_cases_dir = os.path.join(
    ...
    "../../../../test_cases/simulators/docking/CWHSimulator_test_cases/",
)
...
def fixture_expected_state(expected_sim_platforms, expected_sim_entities):
    ...
    clock = 0.0
    state = SafeRLSimulatorState(
        sim_platforms=expected_sim_platforms,
        sim_time=clock,
        sim_entities=expected_sim_entities,
    )
    return state
...
def fixture_expected_sim_platforms(expected_platform_configs):
    ...
    platforms = {
        expected_platform_configs[0]["platform_name"]: CWHPlatformFactory(
            **(expected_platform_configs[0])
        )
    }
    return platforms
...
parameterized_fixture_keywords = [
    "reset_config",
    "expected_platform_configs",
    "step_size",
]
test_configs, IDs = read_test_cases(
    test_cases_file_path, parameterized_fixture_keywords
)


@pytest.mark.unit_test
@pytest.mark.parametrize(
    delimiter.join(parameterized_fixture_keywords), test_configs, ids=IDs, indirect=True
)
...
test_cases_file_path = os.path.join(
    test_cases_dir, "construct_sim_entities_test_cases.yaml"
)
...
test_cases_file_path = os.path.join(
    test_cases_dir, "construct_platforms_test_cases.yaml"
)
parameterized_fixture_keywords = ["expected_platform_configs", "step_size"]
test_configs, IDs = read_test_cases(
    test_cases_file_path, parameterized_fixture_keywords
...
test_cases_file_path = os.path.join(test_cases_dir, "delete_platform_test_cases.yaml")
parameterized_fixture_keywords = ["name", "step_size"]
test_configs, IDs = read_test_cases(
    test_cases_file_path, parameterized_fixture_keywords
)
...
def test_delete_platform(cut, name):
    ...
    assert name not in cut.sim_platforms
    assert name not in cut._state.sim_platforms
    assert expected_sim_entity in cut.sim_entities.values()
    assert expected_sim_entity in cut._state.sim_entities.values()
