...
class SafeAutonomyRllibExperiment(BaseExperiment):
    ...
    def __init__(self, **kwargs) -> None:
        self.config: SafeAutonomyRllibExperimentValidator
    ...
    def run_experiment(self, args: argparse.Namespace) -> None:
        ...
        self.config = self._add_experiment_output_dir(self.config)
    ...
    def _add_experiment_output_dir(self, config: SafeAutonomyRllibExperimentValidator):
        now = datetime.now()

        expr_dir_name = f"expr_{now.strftime('%Y%m%d_%H%M%S')}"

        config.env_config['output_path'] = os.path.join(config.env_config['output_path'], expr_dir_name)
        config.tune_config['local_dir'] = os.path.join(config.tune_config['local_dir'], expr_dir_name)
        return config
